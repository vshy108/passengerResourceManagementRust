...
- `Tier::rank(self) -> u8` (returns 1, 2, or 3)

## Rules (normative)
- **TP-R1**: `Tier::Silver.rank() == 1`, `Tier::Gold.rank() == 2`,
  `Tier::Platinum.rank() == 3`.
...
- **TP-R4**: `Tier` is a closed `enum` with exactly three variants.
  No custom or intermediate tiers exist.
...
|------|----------------|------------------------|
| TP-R1 | TP-S10..S12   | `domain/tier.rs: rank` |
| TP-R2 | TP-S1..S9     | `domain/tier.rs: can_access` |
| TP-E1 | TP-S13..S15   | `domain/tier.rs: TryFrom<&str>` |
